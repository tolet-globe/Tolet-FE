...
export default function MyProperties({ favouriteList = [] }) {
  ...
  useEffect(() => {
    ...
    const fetchMyProperties = async () => {
      try {
        ...
        const response = await Service.getUserProperties(authState.userData.id);
        if (isMounted) {
  ...
  const handleToggleAvailability = useCallback(async (propertyId, currentStatus) => {
    try {
      const response = await Service.updatePropertyAvailabilityStatus(
        propertyId,
        {
          userId: authState?.userData?.id,
          availabilityStatus:
  ...
  }, [myProperties, localFavouriteList, showOption, navigate, handleToggleAvailability, removeFromFavourites, addToFavourites, toggleOption, handleEdit, handleDelete]);
