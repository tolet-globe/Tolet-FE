...
const Listing = () => {
  ...
  const [filters, setFilters] = useState({
    bhk: params.get("bhk") ? params.get("bhk").split(",") : [],
    residential: params.get("residential") ? [params.get("residential")] : [],
    commercial: params.get("commercial") ? [params.get("commercial")] : [],
    preferenceHousing: params.get("preferenceHousing") || "",
    genderPreference: params.get("genderPreference") || "",
    houseType: params.get("houseType") ? params.get("houseType").split(",") : [],
    ownerLocation: params.get("ownerLocation") || "",
  });
  ...
  return (
    <>
      ...
      <div
        ...
        className={`bg-black opacity-80 h-[2600px] absolute z-20 ${isOpen || Hamburger || Location ? "block" : "hidden"
          }`}
      ></div>
      <section
        onClick={() => {
          ...
          Most Trending
              ...
              {isOpen && (
                <div
                  ...
                  className={`fixed inset-0 z-40 flex items-center justify-center bg-black bg-opacity-50 transition-opacity duration-300 ${isOpen ? "opacity-100 visible" : "opacity-0 invisible"
                    }`}
                >
                  <div
                    onClick={(e) => e.stopPropagation()}
                    className={`absolute top-0 right-0 w-full h-full  text-black shadow-lg transition-transform duration-300 ease-in-out transform ${isOpen ? "translate-x-0" : "translate-x-full"
            ...
            <div className="sm:col-span-4 md:col-span-4 flex w-fit xs:w-[50%]  items-center justify-center lg:justify-between -mt-[76px] ml-[98px] xs:[96px] lg:ml-4 lg:mt-0">
              {compareProperty.length >= 1 && (
                <div className="compare relative w-fit ml-20 md:ml-0">
                  ...
                  <button
                    onClick={handleVisit}
                    className={`relative bg-white h-11 sm:h-14 w-20 md:w-32 text-black cursor-pointer rounded-lg flex gap-2 lg:gap-5 text-center items-center px-3 sm:px-7 lg:py-7 text-sm font-medium ${compareProperty.length <= 0
