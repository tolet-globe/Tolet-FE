...
const seeResults = () => {
  ...
  const handleCategoryClick = (category) => {
    ...
    if (["PG", "Flats", "House"].includes(category)) {
      ...
      return; // Do not close the modal, wait for "Done"
    }
    ...
    // Reset residential filters in pending state
    setPendingFilters({
      bhk: [],
      residential: [],
      commercial: [category],
  ...
  // Reusable component to render the filter menus
    ...
    return (
      <div className="w-full lg:bg-white bg-[#232323] py-2 shadow-md sm:ml-4 lg:ml-8 lg:my-2 lg:rounded-xl">
        <div className="flex flex-col gap-5 lg:gap-4">
              ...
              <div className="flex flex-row flex-wrap gap-[2.5rem] pl-[2rem]">
                {options.bhk.map((bhk) => (
                  <label key={bhk} className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-sm lg:text-black text-white overflow-hidden whitespace-nowrap text-ellipsis">{bhk}</span>
                  </label>
                ))}
              </div>
            </div>
          ...
          {options.houseType && (
            <div className="flex flex-col">
              <h3 className="text-left font-medium mb-3 lg:text-black text-white px-4">House Type</h3>
              <div className="flex flex-col md:flex-row lg:flex-row gap-4 pl-[2rem]">
                {options.houseType.map((type) => (
                  <label key={type} className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={() => handlePendingFilterChange("houseType", type)}
          ...
          {options.preference && (
            <div className="flex flex-col">
              <h3 className="text-left font-medium mb-3 lg:text-black text-white px-4">Preference</h3>
              <div className="flex flex-row gap-4 pl-[2rem]">
                {options.preference.map((preference) => (
                  <label key={preference} className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={() => handlePendingFilterChange("preferenceHousing", preference)}
                      className="appearance-none w-4 h-4 rounded-full border-2 border-white lg:border-black checked:bg-black checked:border-[#1890FF] checked:border-4 transition-all"
                    />
          ...
          {options.genderPreference && (
            <div className="flex flex-col">
              ...
              <div className="flex flex-row gap-4 px-4">
                {options.genderPreference.map((gender) => (
                  <label key={gender} className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={() => handlePendingFilterChange("genderPreference", gender)}
                      className="appearance-none w-4 h-4 rounded-full border-2 border-white lg:border-black checked:bg-black checked:border-[#1890FF] checked:border-4 transition-all"
                    />
